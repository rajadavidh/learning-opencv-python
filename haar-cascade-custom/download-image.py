...
# As per April 2020, the image database site is under maintenance
...
def store_raw_images():
    ...
    # neg_image_urls = urllib.request.urlopen(neg_images_link).read().decode()  # Python 3
    ...
    for i in neg_image_urls.split('\n'):
        try:
            ...
            # urllib.request.urlretrieve(i, "neg/" + str(pic_num) + ".jpg")  # Python 3
...
# Steps to run:
# 1. store_raw_images() from image url#1
# 2. store_raw_images() from image url#2
# 3. find_uglies()
# 4. create_pos_n_neg() after the image set is clean
